  ...
  <div id="app">
    ...
    <ul>
      <li v-for="task in tasks" v-bind:key="task.id">
        <input type="checkbox" v-bind:checked="task.done" @change="toggleTaskStatus(task)" />
        {{task.name}}
    ...
    <ul>
      <li v-for="label in labels" v-bind:key="label.id">
        <input type="radio" v-bind:checked="label.id === filter" @change="changeFilter(label.id)" />
        {{label.text}}
      </li>
      <li>
...
export default {
  data() {
    return { newTaskName: "", newTaskLabelIds: [], newLabelText: "" };
  },
  computed: {
    tasks() {
